...
async function resolveDir(dir: string): Promise<{ path: string } | { error: string }> {
  try {
...
server.registerTool(
  "get_second_opinion",
  {
    ...
    inputSchema: {
      ...
      agent: z.enum(["codex", "opencode"]).default("codex").describe(
      ...
      timeout_seconds: z.number().int().positive().max(3600).default(120).optional().describe(
  ...
  async ({ prompt, agent, working_directory, timeout_seconds, output_directory }) => {
    ...
    try {
      ...
      return {
        content: [{ type: "text" as const, text: `Error spawning ${agent}: ${msg}` }],
        isError: true,
      };
    }

    // Surface non-zero exit as error when no useful response was produced
    const failed = result.exitCode !== 0 && result.exitCode !== null && !result.timedOut;
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      return {
        content: [{ type: "text" as const, text: `Failed to write output file: ${msg}` }],
        isError: true,
      };
    }
